...
1. 특이도(Specificity) 2.정확도(Accuracy) 3.민감도(Sensitivity) 4.정밀도(Precision)



- | 혼돈행렬        | Predicted POSITIVE  | Predicted NEGATIVE  |
  | --------------- | ------------------- | ------------------- |
...
- 정확도 : 실제 분류 범주를 정확하게 예측한 비율. 참 긍정(TP)과 참 부정(TN)이 차지하는 비율

- 민감도 : 실제로 '긍정'인 범주 중에서 '긍정'으로 올바르게 예측(TP)한 비율 (TP / (TP + FN))

- 정밀도 : '긍정'으로 예측한 비율 중에서 실제로 '긍정'(TP)인 비율 ( TP / (TP + FP))

- **혼동 행렬**에서 실제로 '부정'인 범주 중에서 '부정'으로 올바르게 예측(TN)한 비율은 특이도이다.
...
### 62. 다음 중 ROC 곡선(Receiver Operating Characteristic Curve; ROC Curve)을 설명한 것으로 가장 부적절한 것은 무엇인가?

1. 가로축($x$)을 혼돈행렬의 거짓 긍정률(FP Rate)로 두고 세로축($y$)을 민감도(TP Rate)로 두어 시각화한 그래프이다. 
2. 거짓 긍정률(FP Rate)과 민감도(TP Rate)는 서로 비례 관계에 있다.
3. AUC(Area Under ROC; AUROC)는 ROC곡선 아래의 면적으로 면적을 모형의 평가지표로 삼는다.
4. ACC의 값은 항상 0.5 ~ 1의 값을 가지며 1에 가까울수록 좋은 모형이다.



- 거짓 긍정률(FP Rate)과 민감도(TP Rate)는 서로 반 비례적인 관계에 있다.

- $$
  거짓 긍정률(FP Rate) = \dfrac {FP}{FP+TN},
  민감도(TP Rate) = \dfrac {TP}{TP+FN}
  $$



### 63. 교차 검증은 모델의 일반화 오차에 대해 신뢰한 추정치를 구하기 위해 훈련, 평가 데이터를 기반으로 하는 검증 기법이다. 다음 중 홀드 아웃 교차 검증을 설명한 것으로 가장 적합한 것은 무엇인가?

1. 전체 데이터에서 평가 데이터를 학습에도 사용하므로 데이터 손실이 발생하지 않는다
2. 전체 데이터를 비복원추출 방식을 이용하여 랜덤하게 학습 데이터(Traning Set)와 평가 데이터(Test Set)로 나눠 검증하는 기법이다.
3. 검증 데이터(Validation Set)는 최적화된 분류기의 성능을 평가할 때 사용하는 데이터이다.
4. 데이터 집합을 무작위로 동일 크기를 갖는 K개의 부분 집합으로 나누고, 그 중 1개를 평가데이터(Test Set)로, 나머지(K-1)개를 학습 데이터(Training Set)로 선정하여 분석 모형을 평가하는 기법이다.
...
- 데이터 집합을 무작위로 동일 크기를 갖는 K개의 부분집합으로 나누고, 그중 1개를 평가 데이터(Test Set)로, 나머지(K-1)개를 학습 데이터(Training Set)로 선정하여 분석 모형을 평가하는 기법은 K-Fold Cross Validation 이다.
- 홀드 아웃 교차 검증은 전체 데이터를 비복원추출 방식을 이용하여 랜덤하게 학습 데이터(Training Set)와 평가 데이터(Test Set)로 나눠 검증하는 기법이다.
...
1. Z-검정 2.T-검정 3.분산 분석 4.카이제곱 검정



- Z 검정 
  - 개념 : 귀무가설에서 검정 통계량의 분포를 정규 분포로 근사할 수 있는 통계 검정이다.
  - 절차 : 정규 분포를 가정하며, 추출된 표본이 동일 모집단에 속하는지 가설을 검증하기 위해 사용한다. 모집단 분산 &sigma;<sup>2</sup> 를 이미 알고 있을 때 분포의 평균을 테스트한다. Z 검정 통계량이 임계치(Critical Value)보다 크고 작음에 따라, 가설을 기각 또는 채택한다.
![Z-Test](https://latex.codecogs.com/svg.image?Z=\dfrac{\bar{x}-\mu}{\dfrac{\sigma}{\sqrt{n}}})



- T-검정 개념
  - 검정하는 통계량이 귀무가설 하에서 T-분포(T-Distribution)을 따르는 통계적 가설검정이다.
  - 두 집단 간의 평균을 비교하는 모수적 토계 방법으로서 표본이 정규성, 등분산성, 독립성 등을 만족할 경우 적용한다.
![T-Test](https://latex.codecogs.com/svg.image?t=\dfrac{\bar{x}-\mu}{\dfrac{s}{\sqrt{n-1}}})

  

- T-분포 특징

  - 정규 분포(Normal Distribution)의 평균을 측정할 때 많이 사용하는 분포이다.
  - 모집단이 정규 분포라는 정도만 알고, &sigma;<sup>2</sup>(모분산)을 모를 때 $s$<sup>2</sup> (표본분산)으로 대체하여 모평균 &mu;를 구할 때 사용한다.
  - 적은 표본으로도 모집단 평균을 추정하려고 정규 분포 대신에 사용되는 확률분포이다.
  - 자유도가 증가할수록 표준 정규 분포에 가까워진다. 예) 자유도 = 표본수 - 1 
  - 자유도가 30이 넘으면 표준 정규 분포와 비슷해지며 이것을 **중심 극한 정리**라고 한다.
  - ![T-Distribution](https://user-images.githubusercontent.com/291782/131966619-df9aa045-72da-4513-8431-10d7f9a83aab.png)

- **분산 분석**(ANOVA; Analysis of Variance)은 두 개 이상의 집단 간 비교를 수행하고자 할 때 집단 내의 분산, 총 평균과 각 집단의 평균 차이에 의해 생긴 집단 간 분산 비교로 얻은 F-분포를 이용하여 가설검정을 수행하는 방법이다.

- **카이제곱검정** 개념

  - 관찰된 빈도가 기대되는 빈도와 유의미하게 다른지를 검정하기 위해 사용되며 카이제곱 분포에 기초한 통계적 검정 방법
  - 단일 표본의 모집단이 정규 분포를 따르며 분산을 알고 있는 경우에 적용한다.
  - 두 집단 간의 동질성 검정에 활용된다.
  - ![Chi-Square Test](https://user-images.githubusercontent.com/291782/131967045-b0a7fcf1-dc48-4d35-ad61-e9fbd117cb55.png)

- 카이제곱 분포

  - 각각 독립인 표준 정규 분포를 취하는 확률변수 $Z$<sub>i</sub> 의 제곱의 합 $X$를 따르는 확률분포이다.
  - 자유도 $n$이 작을수록 왼쪽으로 치우치는 비대칭적 모양이다.
  - 자유도가 $n$>=3 부터 단봉 형태이고, $n$이 클수록 정규 분포에 가까워진다.
  - ![Chi Square distribution](https://user-images.githubusercontent.com/291782/131966404-30972c57-bc48-4f0b-9381-591216a9cc0a.png)
...
![F-Test](https://latex.codecogs.com/svg.image?F=\dfrac{S_1^2}{S_2^2})
  - S_1^2, S_2^2는 표본분산으로 S_1에 큰값, S_2에 작은 값을 넣는다.
...
1. 개별 가중치 값을 제한하여 복잡한 모델을 좀 더 간단하게 하는 방법이다.
2. 드롭아웃은 신경망 학습 시에만 사용하고, 예측시에는 사용하지 않는다.
3. 학습 시에 인공신경망이 특정 뉴런 또는 특정 조합에 너무 의존적이게 되는 것을 방지해준다.
4. 드롭아웃은 학습 과정에서 신경망의 일부를 사용하지 않는 방법이다.





- 개별 가중치 값을 제한하여 복잡한 모델을 좀 더 간단하게 하는 방법은 가중치 규제(Weight Regularization)이다.
...
1. 시간의 변화에 따른 경향(트렌드)을 파악하기 위해 산점도를 사용한다.
2. 집단 간의 상관관계를 확인하여 다른 수치의 변화를 예측하기 위해 히트맵을 사용한다.
3. 지도를 통해 시점에 따른 경향, 차이 등을 확인하기 위해 카토그램을 사용한다.
4. 전체에서 부분 간 관계를 설명하기 위해 막대그래프를 사용한다.



- 해설

| 시간 시각화 | 시간 흐름에 따른 변화를 통해 경향(트렌드) 파악               | 막대그래프<br>점 그래프                       |
| ----------- | ------------------------------------------------------------ | --------------------------------------------- |
| 분포 시각화 | 분류에 따른 변화를 최대, 최소, 전체 분포 등으로 구분<br>전체에서 부분 간 관계를 설명 | 파이 차트<br>도넛차트<br>트리맵               |
| 관계 시각화 | 집단 간의 상관관계를 확인하여 다른 수치의 변화 예측          | 산점도<br>버블 차트<br>히스토그램             |
| 비교 시각화 | 각각의 데이터 간의 차이점과 유사성 관계도 확인 가능          | 히트맵<br>평행 좌표 그래프<br>체르노프 페이스 |
| 공간 시각화 | 지도를 통해 시점에 따른 경향, 차이 등을 확인 가능            | 등치선도<br>도트맵<br>카토그램                |

- 점 그래프
- ![dot-graphe](https://user-images.githubusercontent.com/291782/132031502-0753fd61-8e48-48ab-b721-1f51bb6b6579.png)
- ![tree-map](https://user-images.githubusercontent.com/291782/132031676-88e8ee66-deee-427b-991b-03dba7e274f6.png)
- ![scatter-plot](https://user-images.githubusercontent.com/291782/132031870-8b118b1d-f1da-452a-89ee-ab290ef3545a.png)
- ![histogram](https://user-images.githubusercontent.com/291782/132032179-d425aa47-fa2e-4755-8185-d06078525f9a.png)
- ![hit-map](https://user-images.githubusercontent.com/291782/132032288-1a69dda0-98da-46be-9153-95d57d2e0ee2.png)
- ![parallel-coordinate](https://user-images.githubusercontent.com/291782/132032392-7dc3f974-1a8b-42f2-a1ce-c3ee66be9799.png)
- ![chernope-face](https://user-images.githubusercontent.com/291782/132032541-c5eeb5a9-c721-42ce-9dd6-1b07bfd24d13.png)
- 등치선도 ![deung-chi-sun-do](https://user-images.githubusercontent.com/291782/132032826-7ae91cba-9fff-43be-b354-3f051adcf1ef.png)
- 도트맵(Dot-map) ![dot-map](https://user-images.githubusercontent.com/291782/132033195-5cd4b142-9cb7-4a0f-b424-cd83d7739624.png)
- 카토그램(Cartogram)  ![cartogram](https://user-images.githubusercontent.com/291782/132033312-22448b7e-7162-4de1-9f75-0f9084765343.png)



### 69. 다음 중 빅데이터 분석 결과에 따른 비즈니스 기여도를 평가할 때 사용하는 기법에 대한 설명으로 가장 부적합한 것은 무엇인가?

1. 하나의 자산을 획득하려 할 때 주어진 기간 동안 모든 연관 비용을 고려할 수 있도록 확인하기 위해 총 소유 비용(TCO)을 사용한다.
2. 연 단위 기대수익 규모를 판단하기 위해 순 현재 가치를 "0"으로 만드는 할인율인 내부 수익률(IRR)을 사용한다.
3. 프로젝트의 시작 시점부터 누적 현금흐름이 흑자로 돌아서는 시점까지의 기간을 검토하기 위해 투자 회수 기간(PP)을 사용한다.
4. 투자 타당성을 검토하기 위해 자본 투자에 따른 순 효과의 비율인 순 현재가치(NPV)를 사용한다.



- 자본 투자에 따른 순 효과의 비율을 의미하는 것은 투자 대비 효과(ROI; Return On Investment. 투자수익률)이다.
- 예상 투자비용의 할인가치를 예상 수익의 할인가치에서 공제했을 때 나온 값을 합한 금액이 순 현재가치(NPV; 순현가)이다. NPV: 어떤 사업의 가치를 나타내는 척도 중의 하나. 최초 투자 시기부터 사업이 끝나는 시기까지의 연도별 순편익의 흐름을 각각 현재 가치로 환산한 것. (편익의 현재가치 - 비용의 현재가치 = 순현재가치. 순현재가치 > 0 이면 타당성이 있는 사업으로 판단

> NPV 구하는 법 예) 길동이는 1,000원을 투자해서 총 4년간 각각 100원, 300원, 400원, 500원의 수익이 발생. 이자율은 연 10%이며, 소득세 20% 발생. 
>
> n년차의 수익 = profit x (1 - 소득세율) / (1 + 이자율)<sup>n</sup>
>
> 1년차 수익 : 100 x (1-0.2) / (1+0.1)<sup>1</sup> = 72.7
>
> 2년차 수익 : 300 x (1-0.2) / (1+0.1)<sup>2</sup> = 198.3
>
> 3년차 수익 : 400 x (1-0.2) / (1+0.1)<sup>3</sup> = 240.2
>
> 4년차 수익 : 500 x (1-0.2) / (1+0.1)<sup>4</sup> = 273.2
>
> 총 수익 : 72.7 + 198.3 + 240.2 + 273.2 = 785.1  . 즉 NPV = -214.9 . 하면 안되는 사업
...
1. 무작위로 복원추출하는 기법이다.
2. 전체 데이터에서 중복을 허용한다.
3. 데이터 크기만큼 샘플을 추출하고 이를 학습 데이터(Training Set)로 한다.
4. 샘플에는 한 번도 선택되지 않는 원 데이터가 발생할 수 없다.



- 부트스트랩을 통해 100개의 샘플을 추출하더라도 샘플에 한번도 선택되지 않는 원 데이터가 발생할 수 있다.



### 80. 다음 중 혼돈 행렬을 설명한 것으로 가장 부적합한 것은 무엇인가?

1. 정확도(Accuracy) - 전체 예측에서 참 긍정(TP)과 참 부정(TN)이 차지하는 비율
2. 민감도(Sensitivity) - 실제로 '긍정'인 범주 중에서 '긍정'으로 올바르게 예측(TP)한 비율
3. 정밀도(Precision) - 실제로 '부정'인 범주 중에서 '부정'으로 올바르게 예측(TN)한 비율
4. 거짓 긍정률(FP Rate) - 실제로 '부정'인 범주 중에서 '긍정'으로 잘못 예측(FP)한 비율



- 실제로 '부정'인 범주 중에서 '부정'으로 올바르게 예측(TN)한 비율은 특이도(Specificity)이다.
